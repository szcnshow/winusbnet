    ...
    public class USBPipe
    {
        ...
        private void CheckReadParams(byte[] buffer, int offset, int length)
        {
            ...
            if (offset < 0 || offset >= bufferLength)
                throw new ArgumentOutOfRangeException("offset", "Offset of data to read is outside the buffer boundaries.");
            if (length < 0 || (offset + length) > bufferLength)
                throw new ArgumentOutOfRangeException("length", "Length of data to read is outside the buffer boundaries.");
        }
        private void CheckWriteParams(byte[] buffer, int offset, int length)
        {
            ...
            int bufferLength = buffer.Length;
            if (offset < 0 || offset >= bufferLength)
                throw new ArgumentOutOfRangeException("offset", "Offset of data to write is outside the buffer boundaries.");
            if (length < 0 || (offset + length) > bufferLength)
                throw new ArgumentOutOfRangeException("length", "Length of data to write is outside the buffer boundaries.");
        }
